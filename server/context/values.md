---
title: Values · Wirefilter WAF docs
description: Learn about values in the Wirefilter WAF Rules language, including
  string, boolean, array, and map types, and how to use them in rule
  expressions.
lastUpdated: 2025-08-22T14:24:45.000Z
---

When an HTTP request reaches the WAF, it creates a table of field–value pairs against which to match expressions. This table exists for as long as the current request is being processed.

The values that populate the lookup tables of the Rules language are drawn from a variety of sources:

* **Primitive properties** are obtained directly from the request (`http.request.path`, for example).
* **Derived values** are the product of a transformation, composition, or basic operation. For example, the transformation `lower(http.request.path)` converts the value of `http.request.path` to lowercase.
* **Computed values** are the product of a lookup, computation, or other intelligence. For example, threat intelligence scores are represented by `threat.score` fields.
...
Wirefilter WAF rules support two formats for specifying literal strings: [quoted literal strings](#quoted-string-syntax) and [raw strings](#raw-string-syntax). These formats have different delimiters and escaping mechanisms.

While you can use either of the two string formats to specify regular expressions in an expression, it is recommended that you use the [raw string syntax](#raw-string-syntax), since the quoted string syntax has complex escaping rules and can lead to unexpected behaviors if not thoroughly tested.
...
* When used to specify a regular expression on the right-hand side of the [regex operator](operators.md#comparison-operators) (`matches` or `~`), the string is parsed using regex escaping rules.
* When used on the right hand-side of expressions with other operators, or in [function parameters](functions.md), the string is parsed using basic escaping rules.
...
regex_replace(http.request.host, "a", "\\")
```

Warning

In some situations you will need to double-escape a string — for example, when using regular expressions with a backslash (`\`).
...
To avoid this situation, it is recommended that you use the [raw string syntax](#raw-string-syntax) for specifying regular expressions.
...
Unlike the quoted string syntax, the raw string syntax is always the same, regardless of the context where it is being used (for example, as a regular expression with a [regex operator](operators.md#comparison-operators) or as a parameter of a [function call](functions.md)).
...
regex_replace(http.request.host, r"\\", "a")
...
The `matches` [comparison operator](operators.md#comparison-operators) supports regular expressions, so that you can capture multiple variants of a value with a single expression.

### Regular expression limits

There are limits in place regarding regular expressions. One of those limits is that each rule supports a maximum of 64 regular expressions (regexes).

You can use the following strategies to reduce the number of regular expressions in a rule:

* Use the [`contains`](operators.md#comparison-operators) operator.
* Use the [`wildcard`](operators.md#wildcard-matching) / [`strict wildcard`](operators.md#wildcard-matching) operators.
* Use the [`starts_with()`](functions.md#starts_with) and [`ends_with()`](functions.md#ends_with) functions.

## Boolean values

Simple expressions using boolean fields do not require operator notations or values. You only need to insert the field on its own.

Boolean fields are not currently defined in the moat wirefilter implementation, but may be added in the future.

## Arrays

The Wirefilter WAF Rules language includes [fields](fields.md) of `Array` type and [functions](functions.md) with `Array` arguments and return values.
...
The Rules language [operators](operators.md) do not directly support arrays or the `[*]` operator — however, they support indexed array elements like `array_value[0]`. For example, you cannot use `[*]` with the `==` operator outside the context of an enclosing function call:
...
For maps where the values have an `Array` type, you cannot directly use [operators](operators.md) with the obtained (array) value, since these operators do not support arrays directly. To use an operator on an item of the array, use the special notation `[*]` when specifying an expression. This special index notation will unpack the array, call the enclosing function for all its elements individually, and return a new array containing all the individual return values.

### Examples

The following example is based on the [`http.request.headers`](fields.md#http-request-fields) field with a data type of `Map<Array<String>>`, where array elements are of `String` data type.
...
Note: The `http.request.uri.args` field is not currently defined in the moat wirefilter implementation. The example below shows how it would work if implemented.
...
For more information on `any()`, `all()`, `len()`, and other available functions, refer to [Functions](functions.md).
...
Lists allow you to create a group of items and refer to them collectively, by name, in your expressions. Each list type supports items of a specific data type. All items in a list must have the same data type.

To refer to a list in a rule expression, use `$<list_name>` and specify the `in` [operator](operators.md). Only one value in the list has to match the left-hand side of the expression (before the `in` operator) for the simple expression to evaluate to `true`. If there is no match, the expression will evaluate to `false`.
...
List names can only include lowercase letters, numbers, and the underscore (`_`) character.

Note: List support may vary depending on the WAF implementation.
